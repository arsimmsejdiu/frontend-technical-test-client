...
describe("routes/_authentication/index", () => {
  describe("MemeFeedPage", () => {
    ...
    it("should add a comment", async () => {
      const mockCreateMeme = vi.fn().mockResolvedValue({
        id: 'dummy_id',
        pictureUrl: 'dummy_url',
        description: 'dummy_description',
        texts: [],
        createdAt: new Date().toISOString(),
      });
      vi.mocked(createMeme).mockImplementation(mockCreateMeme);

      renderMemeFeedPage();

      await waitFor(() => expect(screen.getByText("dummy meme 1")).toBeInTheDocument());
      ...
      await waitFor(() => {
        expect(mockCreateMeme).toHaveBeenCalledWith("dummy_token", "dummy_meme_id_1", "Test comment");
      });
